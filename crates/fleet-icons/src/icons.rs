...
//! # Redis Caching
//!
//! When the `redis-cache` feature is enabled, generated icons are cached in Redis
//! with a configurable TTL. This persists icons across gateway restarts and allows
//! multiple gateway instances to share the same icon cache.
...
/// Redis key prefix for cached icons.
#[cfg(feature = "redis-cache")]
const REDIS_KEY_PREFIX: &str = "fleet:icon:";

/// Default TTL for cached icons in seconds (24 hours).
#[cfg(feature = "redis-cache")]
const REDIS_TTL_SECONDS: u64 = 86400;
...
impl IconCache {
    ...
    pub fn insert(&mut self, key: String, data_uri: String) {
        self.icons.insert(key, data_uri);
    }
    ...
    /// Return all cached icons as a BTreeMap.
...
pub fn icon_key(ship_type: i32, mode: MarkerMode) -> String {
...
pub fn vessel_id(vessel: &Vessel) -> String {
    format!("{:.6},{:.6},{}", vessel.lat, vessel.lon, vessel.ship_type)
}

// ---------------------------------------------------------------------------
// Redis-backed persistent cache (optional)
// ---------------------------------------------------------------------------

/// A Redis-backed icon cache that persists generated SVG icons across restarts.
///
/// Icons are stored with a 24-hour TTL. The cache is safe to share across
...
impl RedisIconCache {
    ...
    ///
    /// Returns `None` if the connection cannot be established — callers should
    ...
    /// Store an icon with the default TTL.
    #[cfg(feature = "redis-cache")]
    pub fn set(&self, key: &str, data_uri: &str) {
    ...
    pub fn is_available(&self) -> bool {
        self.client
            .get_connection()
            .and_then(|mut conn| redis::cmd("PING").execute(&mut conn).map(|_| true))
            .unwrap_or(false)
    }
}
...
mod tests {
    ...
    fn different_types_get_different_icons() {
        ...
        assert_eq!(
            cache.len(),
            2,
            "different types must produce different icons"
        );
    }
